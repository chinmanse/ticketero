...
class RespuestaService():
  def index():
    ...
    users = session.query(RespuestaModel).all()
    return users
  ...
  def get_by_ticket(ticketId):
    session = SessionLocal()
    respuestas = session.query(RespuestaModel).\
      options(joinedload(RespuestaModel.ticket), joinedload(RespuestaModel.user)).\
      filter(RespuestaModel.ticket_id == ticketId).\
      all()
    return respuestas
